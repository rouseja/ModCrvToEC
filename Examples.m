...
tim1 := Cputime(tim0);
printf "There are %o rational points on 6.6.1.a.1. The multiset of j-images is %o. Total time taken was %o sec.\n",#ratpoints,jims,tim1;
...
ModEC := InitializeModEC(N, gens : precmult := 1.6208);
...
jims := RatPtsJInvs(ModEC,ratpoints);
tim1 := Cputime(tim0);
printf "The images on the j-line are %o. Total time taken was %o sec.\n",jims,tim1;
...
// Run time is about 130 seconds.

N := 18;
gens := [[5,8,12,5],[14,9,9,4],[17,3,0,1]];
tim0 := Cputime();
// We need some extra precision to compute Hecke operators T_5 through T_47.
ModEC := InitializeModEC(N, gens : precmult := 3, Verbose := true);  // Setting precmult to 2 doubles the default precision.
pts := PointSearch(ModEC`XG,100);
ModEC`BasePt := pts[1];
Iso, Mx := EllipticCurveQuoCandidates(ModEC : OnlyRankZero := true);
Maps := FindMapsToEC(ModEC, Iso, Mx); 
