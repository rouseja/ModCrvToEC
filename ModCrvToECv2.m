...
RealizeInFF := function(ModEC)
  ...
  while not (&and donelist) do
    ...
    /* The next two lines could be very slow in high genus
    mons := MonomialsOfDegree(polyring,d);
    bas := [ m : m in mons | not (m in initideal)]; */    
    if (d eq 1) then
      bas := [ m : m in MonomialsOfDegree(polyring,1) | not (m in initideal)];
    else
      bas := Sort(SetToSequence({ r*m : r in canring[d-1][2], m in MonomialsOfDegree(polyring,1) | not (r*m in initideal)}));
    end if;
    newfourier := <>;
    newvars := [];
    if (d eq 1) then
  ...
  while not (&and donelist) do
    ...
    if #canring lt d then
      ...
      if (d eq 1) then
        ...
        bas := Sort(SetToSequence({ (polyring!r)*m : r in canring[d-1][2], m in MonomialsOfDegree(polyring,1) | not ((polyring!r)*m in initideal)}));
      end if;
      newfourier := <>;
      newvars := [];
      if (d eq 1) then
...
// Our new, mostly custom function for pulling back rational points. The input is the ModEC record
// returned by FindMapsToEC
...
intrinsic RatPtsFromMaps(ModEC : Verbose := false) -> SeqEnum
  ...
  N := ModEC`N;
    ...
    if Verbose then
      printf "Map to elliptic curve has %o defining polynomials.\n",#ModEC`ecmaps;
    end if;
    assert Rank(E) eq 0;
    if Verbose then
    ...
    polring := Parent(Generators(Ideal(X))[1]);
    numvars := Rank(polring);
    for P in ptsE do
      // Keep track of potential primes of bad reduction of the map
      N2 := Integers()!N;
      if Verbose then
      ...
      sing := true;
      maxpts := -1;
      polylist := Generators(Ideal(X));
      // Manually build the preimage scheme.
      for k in [1..#ModEC`ecmaps] do
        if Eltseq(P) ne [0,1,0] then
          Append(~polylist,ModEC`ecmaps[k][1]-P[1]*ModEC`ecmaps[k][3]);
          Append(~polylist,ModEC`ecmaps[k][2]-P[2]*ModEC`ecmaps[k][3]);
        else
          Append(~polylist,ModEC`ecmaps[k][1]);
          Append(~polylist,ModEC`ecmaps[k][3]);
        end if;
      end for;
      X2 := Scheme(ProjectiveSpace(Rationals(),numvars-1),polylist);
      polylist2 := [];
      foundall := false;
      foundpoints := [];
      for q in polylist do
        ...
        N2 := Integers()!(N2*denom);
      end for;
      curp := 1;
      step2done := false;
      while (step2done eq false) do
        curp := NextPrime(curp);
        if (GCD(curp,N2) eq 1) then
          if Verbose then
            printf "Beginning test of p = %o.\n",curp;
          end if;
          ...
          if Dimension(X2p) eq 0 then
            ...
            if Verbose then
              printf "Tested p = %o. There are %o points, of which %o are singular.\n",curp,#pts,#singpts;
            end if;
